...
import {
  Container, Typography, Card, CardContent, Box, Chip, LinearProgress,
  Avatar, Button, Divider, Stack, Paper, Grid
} from '@mui/material';
...
export default function ApplicantsDashboard() {
  ...
  const ApplicantCard = ({ app }) => (
    <Card elevation={6} sx={{ borderRadius: 3, mb: 3, border: app.parsedData.isShortlisted ? '3px solid #4caf50' : '1px solid #e0e0e0' }}>
      <CardContent>
        <Grid container spacing={3}>
            ...
            <Box display="flex" alignItems="center" gap={2}>
              ...
              <Box>
                ...
                <Typography color="text.secondary">
                  {app.parsedData.email} {app.parsedData.phone && `• ${app.parsedData.phone}`}
                </Typography>
                <Typography variant="body1" sx={{ mt: 1 }}>
                  Applied for: <strong>{app.job.title}</strong> ({app.job.department})
                </Typography>
              </Box>
            </Box>
          </Grid>
          ...
          <Grid item xs={12} md={4}>
            <Box textAlign={{ xs: 'left', md: 'right' }}>
              <Chip
                icon={app.parsedData.isShortlisted ? <CheckCircle /> : <Cancel />}
                label={app.parsedData.isShortlisted ? 'SHORTLISTED' : 'APPLIED'}
                color={app.parsedData.isShortlisted ? 'success' : 'default'}
                size="small"
              />
              <Typography variant="h3" fontWeight="bold" color={app.parsedData.matchPercentage >= 70 ? 'success.main' : 'warning.main'}>
              ...
              <LinearProgress
                ...
                sx={{
                  ...
                  '& .MuiLinearProgress-bar': {
                    bgcolor: app.parsedData.matchPercentage >= 70 ? '#4caf50' : app.parsedData.matchPercentage >= 50 ? '#ff9800' : '#f44336'
                  }
                }}
              />
            </Box>
        ...
        <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
          Matched Skills ({app.parsedData.matchedSkills.length} of {app.job.skills.length})
        </Typography>
        <Stack direction="row" flexWrap="wrap" gap={1}>
          ...
          {app.parsedData.missingSkills.map(skill => (
            <Chip key={skill} label={skill} color="error" variant="outlined" size="small" />
          ))}
        </Stack>
...
>
  ...
  <Typography variant="h6" color="success.dark">
    These candidates match ≥70% of required skills
      ...
      {shortlisted.map(app => <ApplicantCard key={app._id} app={app} />)}

      <Typography variant="h5" sx={{ mt: 8, mb: 3 }} color="text.secondary">
        Other Applicants ({others.length})
      </Typography>
      {others.map(app => <ApplicantCard key={app._id} app={app} />)}
    </Container>
  );
}